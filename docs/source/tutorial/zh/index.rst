...
* **语法解析**：此部分主要调用的是parse模块，其作用是将传入的item转换为标准sif格式（即把字母、数字用 ``$...$`` 包裹起来，把选择填空的括号、下划线转换为特殊符号等），从而方便后期的处理。

* **成分分解**：此部分主要调用的是segment模块，其作用是将传入的符合sif标准的item根据元素种类进行分割开来，从而服务于后面的令牌化环节（即可以将不同类型元素使用各自的方法令牌化）。

* **令牌化**：此部分主要调用的是tokenize模块，其作用是将传入的经过分词后的item元素列表进行令牌化分解，从而服务于后面的向量化模块。
  其中通常情况下直接使用文本形式的令牌化方法即可，对于公式而言还可使用ast方法进行解析(调用formula模块)；
  此外，本项目还提供了令牌化容器，从而方便用户快速获得item相应的令牌化结果。
