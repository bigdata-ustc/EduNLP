...
class GensimSegTokenizer(object):  # pragma: no cover
    ...
    >>> print(token_item[:10])
    [['公式'], [\FormFigureID{wrong1?}], ['如图'], ['[FIGURE]'], ['mathord', ',', 'mathord'], ['约束条件', '公式'], [[FORMULA]], ['mathord', '=', 'mathord', '+', 'textord', 'mathord'], ['最大值'], ['[MARK]']]
    >>> tokenizer = GensimSegTokenizer(symbol="fgm", depth=None)
    >>> token_item = tokenizer("有公式$\\FormFigureID{wrong1?}$，如图$\\FigureID{088f15ea-xxx}$,若$x,y$满足约束条件公式$\\FormFigureBase64{wrong2?}$,$\\SIFSep$，则$z=x+7 y$的最大值为$\\SIFBlank$")
    >>> print(token_item[:10])
    [['公式'], ['[FORMULA]'], ['如图'], ['[FIGURE]'], ['[FORMULA]'], ['约束条件', '公式'], ['[FORMULA]'], ['[FORMULA]'], ['最大值'], ['[MARK]']]
    """
