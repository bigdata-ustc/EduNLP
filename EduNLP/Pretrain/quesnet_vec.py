...
Question = namedtuple(
    "Question", ["id", "content", "answer", "false_options", "labels"]
)
...
class QuesNetTokenizer(PretrainedEduTokenizer):
    ...
    ... trim_min_count=1, key=lambda x: x["ques_content"], silent=True) # doctest: +ELLIPSIS
    [[...]]
    ...
    def set_meta_vocab(self, items: list, meta: List[str] = None, silent=True):
        ...
        for m in self.meta:
            ...
            if not silent:
                logger.info(f"save meta information {m}: {len(meta)}")
            self.stoi[m] = {word: index for index, word in enumerate(meta)}
            self.itos[m] = {i: s for s, i in self.stoi[m].items()}
    ...
    def set_vocab(
        ...
        token_items = self.set_text_vocab(
            items,
            key=key,
            lower=lower,
    ...
    def produce(self):
        for i in range(self.pos, len(self.index)):
            try:
                ...
                label_batch = [
                    label[index * bs: (index + 1) * bs] for label in self.label
                ]
                if label_batch:
...
def pretrain_quesnet(
    ...
    ... }]
    >>> tokenizer.set_text_vocab(items,
    ... key=lambda x: x['ques_content'], trim_min_count=1, silent=True) # doctest: +ELLIPSIS
    [[...]]
    ...
    model = QuesNetForPreTraining(
        _stoi=tokenizer.stoi,
        feat_size=work_model_params["feat_size"],
        ...
        for m in qs.labels[model.quesnet.meta]:
            meta_vector.add_(
                torch.nn.functional.one_hot(
                    torch.tensor(m, dtype=torch.int64), model.quesnet.meta_size
                ).to(torch.float)
            )
    ...
    if pretrain_dir and load_embs and os.path.exists(pretrained_model_path):
        ...
        logger.info(
            f"Load QuesNetForPreTraining from checkpoint: {pretrained_model_path}"
        )
    ...
    for epoch in range(0, work_train_params["n_epochs"]):
        ...
        bar = enumerate(tqdm(train_iter, initial=train_iter.pos), train_iter.pos)
