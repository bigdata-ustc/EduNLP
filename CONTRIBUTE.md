...
2. Inherit the `class NLP` in `EduNLP/meta.py` and implement four methods in it.
3. Write the corresponding example code for the model (This refers to a demo that can be tested by others). It should include at least [notebook](examples/sif/sif.ipynb) and [script](examples/sif/sci4sif.py). [An example](examples/sif).
4. Write the corresponding test code for the model and make sure that the test coverage is 100%. [An example](examples/sif).
...
All modules are inherited from `Class NLP`, it will raise `NotImplementedError` if the functions are not implemented.
