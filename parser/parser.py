class Parser:
    ...
    def get_token(self):
        ...
        if self.is_chinese(ch):
            ...
            left = head = self.head
            if self.head == 0:
                while (head < len(self.text) and (self.is_number(self.text[head]) or self.text[head] in self.in_list)):
                    head += 1
                if head == len(self.text) or self.is_chinese(self.text[head]) or self.text[head] in self.flag_list:
                    self.head = head
                    self.text = self.text[:left] + "$" + self.text[left:head] + "$" + self.text[head:]
            ...
            left = self.head
            self.head += 1
            if (self.text[left] == '（'):  # 匹配到一个左括号
                while (self.text[self.head] == ' ' or self.text[self.head] == '\u3000' or self.text[self.head] == '\xa0'):
                    self.head += 1
            ...
            if self.text[left] == '(':  # 匹配到一个左括号
                ...
                if (self.text[self.head] == ')'):
                    ...
                    self.modify_flag = 1
                    return self.modify
            if self.text[left] == '_':  # 匹配到一个下划线
                #                 print('this is an underline')
                while (self.text[self.head] == '_' or self.text[self.head] == ' ' or self.text[self.head] == '\xa0'):
                    self.head += 1
                    if self.head >= len(self.text):
                        break
